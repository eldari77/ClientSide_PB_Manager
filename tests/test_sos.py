...
def test_expand_sos_bridge_configs_applies_mode_policy_and_services(tmp_path: Path):
    ...
    assert config.child_worker_scripts[20]["service_id"] == "readiness"
    assert config.child_worker_scripts[20]["budget"] == 1
    assert config.child_worker_scripts[20]["priority"] == 4
    assert config.child_worker_scripts[21]["budget"] == 0
    assert config.child_worker_scripts[22]["expires_after_sequences"] == 1
